...
class SlopeCloudFilter(Node):
    def __init__(self):
        ...
        # 로봇 차체에서 반사된 점을 누적 전에 제거한다.
        # 좌표는 입력 PointCloud의 BODY frame 기준이다.
        self.declare_parameter('self_filter_enabled', True)
        self.declare_parameter('self_filter_min_x_m', -0.60)
        self.declare_parameter('self_filter_max_x_m', 0.50)
        self.declare_parameter('self_filter_min_y_m', -0.35)
        self.declare_parameter('self_filter_max_y_m', 0.35)
        ...
        self.self_filter_enabled = bool(gp('self_filter_enabled').value)
        self.self_filter_min_x_m = float(gp('self_filter_min_x_m').value)
        self.self_filter_max_x_m = float(gp('self_filter_max_x_m').value)
        self.self_filter_min_y_m = float(gp('self_filter_min_y_m').value)
        self.self_filter_max_y_m = float(gp('self_filter_max_y_m').value)
        ...
        self.get_logger().info(
            f'Self filter : {self.self_filter_enabled}  '
            f'x=[{self.self_filter_min_x_m:.2f}, {self.self_filter_max_x_m:.2f}] m, '
            f'y=[{self.self_filter_min_y_m:.2f}, {self.self_filter_max_y_m:.2f}] m'
        )
    ...
    def on_param_change(self, params):
        ...
        for p in params:
            ...
            try:
                ...
                elif n == 'self_filter_enabled':
                    self.self_filter_enabled = bool(p.value)
                elif n == 'self_filter_min_x_m':
                    self.self_filter_min_x_m = float(p.value)
                elif n == 'self_filter_max_x_m':
                    self.self_filter_max_x_m = float(p.value)
                elif n == 'self_filter_min_y_m':
                    self.self_filter_min_y_m = float(p.value)
                elif n == 'self_filter_max_y_m':
                    self.self_filter_max_y_m = float(p.value)
    ...
    def cloud_callback(self, msg):
        ...
        # 자기 차체 포인트는 odom 보정 누적 전에 제거해야 한다.
        # 누적 후 제거하면 과거 자세의 차체 점이 현재 차체 박스 밖으로
        # 이동해 외부 장애물처럼 남게 된다.
        self_removed_count = 0
        if self.self_filter_enabled:
            self_mask = (
                (xyz[:, 0] >= self.self_filter_min_x_m)
                & (xyz[:, 0] <= self.self_filter_max_x_m)
                & (xyz[:, 1] >= self.self_filter_min_y_m)
                & (xyz[:, 1] <= self.self_filter_max_y_m)
            )
            self_removed_count = int(np.count_nonzero(self_mask))
            xyz = xyz[~self_mask]
            if len(xyz) == 0:
                return
        ...
        if now - self.last_log_time >= self.log_period_sec:
            ...
            self.get_logger().info(
                ...
                f'Scan={raw_count} (self -{self_removed_count}) '
                f'x{len(self.scan_buf)} -> {accum_count} | '
